...
urlpatterns = [
    ...
    path('api/auth/refresh/', TokenRefreshView.as_view(permission_classes=[
         AllowAny]), name='token_refresh'),  # Usamos la de simplejwt

    # --- Endpoints de flujo de Registro ---
    path('api/auth/registrar-usuario/',
         UsuarioRegistroView.as_view(), name='registrar_usuario'),
    path('api/auth/registrar/activar-cuenta/',
         ActivarCuentaView.as_view(), name='activar_cuenta'),

    # --- Flujo de Recuperación de Cuenta ---
    path('api/auth/recuperar/solicitar-codigo/',
         SolicitarCodigoView.as_view(), name='solicitar_codigo_verificacion'),
    path('api/auth/recuperar/restablecer-contraseña/',
         RestablecerContraseñaView.as_view(), name='restablecer_contraseña'),
    path('api/auth/recuperar/recuperar-username/',
