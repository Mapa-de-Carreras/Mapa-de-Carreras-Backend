...
class AsociarAsignaturaAPlanView(APIView):
    """
    ...
    @swagger_auto_schema(
        tags=["Gestión Académica - Planes de Estudio"],
        operation_summary="Asociar Asignatura a un Plan de Estudio",
        operation_description=(
            "Permite asociar una asignatura existente a un plan de estudio.\n\n"
            "⚠️ No se permite asociar asignaturas inactivas o duplicadas dentro del mismo plan."
        ),
        request_body=PlanAsignaturaSerializer,
        responses={
            201: "Asignatura asociada correctamente al plan.",
            400: "Error de validación o asignatura duplicada.",
        },
    )
    def post(self, request):
        serializer = PlanAsignaturaSerializer(data=request.data)
        if serializer.is_valid():
            plan_asignatura = plan_de_estudio.asociar_asignatura_a_plan(serializer.validated_data)
            return Response({
                "message": "Asignatura asociada correctamente al plan de estudio.",
                "data": PlanAsignaturaSerializer(plan_asignatura).data
            }, status=status.HTTP_201_CREATED)

        return Response({
            "message": "Error al asociar la asignatura al plan de estudio.",
            "errors": serializer.errors
        }, status=status.HTTP_400_BAD_REQUEST)


class DesasociarAsignaturaDePlanView(APIView):
    """
    ...
    @swagger_auto_schema(
        tags=["Gestión Académica - Planes de Estudio"],
        operation_summary="Desasociar Asignatura de un Plan de Estudio",
        operation_description=(
            "Permite desasociar una asignatura específica de un plan de estudio.\n\n"
            "⚠️ Solo coordinadores de la carrera o administradores pueden realizar esta acción."
        ),
        manual_parameters=[
            openapi.Parameter(
                "plan_id",
            ...
            openapi.Parameter(
                "asignatura_id",
                openapi.IN_PATH,
                description="ID de la asignatura a desasociar",
                type=openapi.TYPE_INTEGER,
                required=True
            ),
        ],
        responses={
            200: "Asignatura desasociada correctamente.",
            404: "No se encontró la asociación entre la asignatura y el plan.",
        },
    )
    def delete(self, request, plan_id, asignatura_id):
        eliminado = plan_de_estudio.desasociar_asignatura_de_plan(plan_id, asignatura_id)
        if not eliminado:
            return Response({
                "message": "No se encontró la asociación entre la asignatura y el plan de estudio."
            }, status=status.HTTP_404_NOT_FOUND)
        return Response({
            "message": "Asignatura desasociada correctamente del plan de estudio."
        }, status=status.HTTP_200_OK)
